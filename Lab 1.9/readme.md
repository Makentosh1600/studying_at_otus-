...
│   ┌─────────────────┐ G0/0/1              Fa0/5         Fa0/1  ┌─────┐ │
│   │      R1         ├──────────────────────────────────────────┤ S1  │ │
│   │   (DHCP Relay)  │            (10.53.0.0/24)               │     │ │
│   │                 │              VLAN 10                     └─────┘ │
│   │ Lo0: 10.10.1.1  │              Management                    │    │
│   │ G0/0/1:         │                                       Fa0/6│    │
│   │ 192.168.10.1    │                                            │    │
│   └─────────────────┘                                       ┌─────────┐ │
│          │                                                 │  PC-A   │ │
...
| Устройство | Тип | Версия ПО | Количество | Назначение |
|-----------|------|-----------|-----------|-----------|
| R1 | Cisco 4221 | IOS XE 16.9.3 | 1 | DHCP-сервер, шлюз сети |
| S1 | Cisco Catalyst 2960 | IOS 15.0(2) | 1 | Основной коммутатор |
| S2 | Cisco Catalyst 2960 | IOS 15.0(2) | 1 | Вторичный коммутатор |
| PC-A | Windows | --- | 1 | DHCP-клиент (VLAN 10) |
| PC-B | Windows | --- | 1 | DHCP-клиент (VLAN 10) |
...
|-----------|-----------|----------|---------------|-----------|
| **R1** | G0/0/1 | 192.168.10.1 | 255.255.255.0 | Шлюз VLAN 10 |
| **R1** | Loopback0 | 10.10.1.1 | 255.255.255.0 | Тестовая сеть |
| **S1** | VLAN 10 | 192.168.10.201 | 255.255.255.0 | Управление S1 |
| **S2** | VLAN 10 | 192.168.10.202 | 255.255.255.0 | Управление S2 |
| **PC-A** | NIC | DHCP | 255.255.255.0 | Тестовый клиент |
| **PC-B** | NIC | DHCP | 255.255.255.0 | Тестовый клиент |

---

## 📋 Конфигурация VLAN

| VLAN ID | Имя | Назначение | Активные порты |
|---------|------|-----------|---------------|
| 1 | Default | VLAN по умолчанию | (не используется) |
| 10 | Management | Управление и клиенты | S1: F0/5, F0/6; S2: F0/18 |
| 333 | Native | Native VLAN для trunk | --- |
| 999 | ParkingLot | Заблокированные порты | S1/S2: остальные порты |

---

## 💾 Необходимые ресурсы

- **1 маршрутизатор:** Cisco 4221 с Cisco IOS XE Release 16.9.3 (образ universalk9) или аналогичный
- **2 коммутатора:** Cisco Catalyst 2960 с Cisco IOS версии 15.0(2) (образ lanbasek9) или аналогичные
- **2 ПК:** Windows с программой эмуляции терминала (Tera Term, PuTTY или подобная)
- **Консольные кабели:** Для конфигурации устройств через консольные порты
- **Кабели Ethernet:** Как указано в топологии

**⚠️ ВАЖНО:** Перед началом работы убедитесь, что все маршрутизаторы и коммутаторы имеют очищенную конфигурацию и отсутствует загрузочная конфигурация. При необходимости обратитесь к инструктору.
...
Подключите устройства согласно топологии:
- R1 G0/0/1 ↔ S1 F0/5 (консольный кабель для конфигурации)
- S1 F0/1 ↔ S2 F0/1 (trunk link)
- S1 F0/6 ↔ PC-A (консоль или NIC)
- S2 F0/18 ↔ PC-B (консоль или NIC)
- Используйте консольные кабели для прямого подключения ПК к маршрутизаторам/коммутаторам
...
! Конфигурация DHCP пула для студентов
...
! Конфигурация Loopback0 интерфейса (имитация сетевого сегмента)
interface Loopback0
ip address 10.10.1.1 255.255.255.0
exit

! Конфигурация основного интерфейса
...
! Конфигурация консольного порта
line con 0
logging synchronous
exec-timeout 0 0
exit
...
**Ожидаемый результат:**
- Интерфейс GigabitEthernet0/0/1 в состоянии UP с IP 192.168.10.1
- Интерфейс Loopback0 в состоянии UP с IP 10.10.1.1

### 1.4 Базовая конфигурация коммутаторов S1 и S2
...
! Установка шлюза по умолчанию (для VLAN управления)
ip default-gateway 192.168.10.1

! Описания используемых интерфейсов (S1)
interface F0/5
...
! Описания используемых интерфейсов (S2)
! interface F0/1
! description Link to S1
! exit
...
## 📝 ЧАСТЬ 2: Настройка и управление виртуальными локальными сетями (VLAN)

### 2.1 Создание VLAN 10 (Management)
...
! Создание VLAN 333 (Native VLAN для trunk)
vlan 333
name Native
exit

! Создание VLAN 999 (Parking Lot для неиспользуемых портов)
vlan 999
name ParkingLot
exit
...
### 2.2 Настройка интерфейса управления (SVI) для VLAN 10
...
### 2.3 Проверка конфигурации VLAN
...
**Ожидаемый результат:**
- VLAN 10 (Management) активна
- VLAN 333 (Native) активна
- VLAN 999 (ParkingLot) активна
