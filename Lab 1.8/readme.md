...
| Устройство | Интерфейс | IPv6-адрес | Описание |
|-----------|-----------|-----------|---------|
| R1 | G0/0/0 | 2001:db8:acad:2::1/64 | Link к R2 |
| R1 | G0/0/0 | fe80::1 | Link-local R1-R2 |
| R1 | G0/0/1 | 2001:db8:acad:1::1/64 | Link к PC-A (SLAAC) |
| R1 | G0/0/1 | fe80::1 | Link-local R1-PC-A |
| R2 | G0/0/0 | 2001:db8:acad:2::2/64 | Link к R1 |
| R2 | G0/0/0 | fe80::2 | Link-local R2-R1 |
| R2 | G0/0/1 | 2001:db8:acad:3::1/64 | Link к PC-B (DHCPv6) |
| R2 | G0/0/1 | fe80::1 | Link-local R2-PC-B |
| PC-A | NIC | DHCP | Получение конфигурации от R1 |
| PC-B | NIC | DHCP | Получение конфигурации через Relay |
...
- ✅ Настройки базовой топологии IPv6
- ✅ Конфигурирования SLAAC (Stateless Address AutoConfiguration)
- ✅ Развертывания DHCPv6 Stateless сервера
- ✅ Развертывания DHCPv6 Stateful сервера
- ✅ Настройки DHCPv6 Relay агента
...
Подключите устройства согласно диаграмме:
- PC-A → S1 (Fast Ethernet 0/6)
- S1 (F0/5) → R1 (G0/0/1)
- R1 (G0/0/0) ↔ R2 (G0/0/0)
- R2 (G0/0/1) → S2 (F0/5)
- S2 (F0/18) → PC-B
...
## Часть 2: Проверка назначения адреса SLAAC

### 2.1 Принцип работы SLAAC

**SLAAC** (Stateless Address AutoConfiguration) позволяет хостам автоматически генерировать IPv6 адреса на основе:
- Префикса маршрутизатора (первые 64 бита)
- Уникального идентификатора хоста на базе MAC-адреса

### 2.2 Генерация идентификатора интерфейса (IID)
